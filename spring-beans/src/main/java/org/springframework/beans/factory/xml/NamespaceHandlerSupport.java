...
public abstract class NamespaceHandlerSupport implements NamespaceHandler {
	...
	public BeanDefinition parse(Element element, ParserContext parserContext) {
		//以<context:component-scan这个标签为例，由于 ContextNamespaceHandler这个类执行了init方法，因此会有一个存放key,value的集合容器
		//其中key为context:后面的参数，因此得到了 component-scan 通过component-scan可以拿到 ComponentScanBeanDefinitionParser 这个类
		BeanDefinitionParser parser = findParserForElement(element, parserContext);
		//然后执行ComponentScanBeanDefinitionParser 的 parse方法，这边也使用了策略模式
	...
	private BeanDefinitionParser findParserForElement(Element element, ParserContext parserContext) {
		// <context:component-scan base-package="com.xiangxue"/>  拿到  component-scan
